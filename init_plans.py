...
if supabase_url and supabase_key:
    ...
    CREATE POLICY "Apenas masters podem editar planos" ON public.subscription_plans FOR ALL USING (
        EXISTS (
            ...
            AND (profiles.role = 'master' OR profiles.role = 'admin' OR profiles.email LIKE '%aline%')
        )
    );
    ...
    try:
        res = client.rpc('exec_sql', {'sql_string': sql}).execute()
        print('Executed SQL:', res)
    except Exception as e:
        print(f'Error executing SQL: {e}')
else:
    print('No credentials found in env. Falling back to migration generation.')
