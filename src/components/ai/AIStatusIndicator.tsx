...
export function AIStatusIndicator() {
    ...
    return (
      <>
        <button 
          ...
          className="fixed bottom-6 right-6 z-40 w-12 h-12 bg-zinc-900 hover:bg-zinc-800 text-white rounded-full shadow-lg flex items-center justify-center transition-transform hover:scale-105"
  ...
  return (
    <>
      <button 
        onClick={() => setDashboardOpen(true)}
        className={`fixed bottom-6 right-6 z-40 flex items-center gap-3 px-4 h-12 rounded-full shadow-lg transition-all hover:scale-105 ${requiresAttention ? 'bg-red-500 hover:bg-red-600' : 'bg-vj-green hover:bg-vj-green/90'} text-white`}
      >
