...
export const CardPromotionBlock = {
 ...
 {node.props.badgeText || 'Special Offer'}
 </div>
 </div>
 <div className="max-w-xs relative z-10">
 <h3 className="text-2xl font-bold mb-2">{node.props.title || 'Summer Getaway'}</h3>
 <p className="opacity-90 mb-6">{node.props.description || 'Book now and get up to 30% off on all premium packages.'}</p>
 <button className="bg-white text-indigo-700 font-semibold px-6 py-2 rounded-lg hover:bg-gray-50 transition-colors ">
 {node.props.buttonText || 'Claim Discount'}
 </button>
 </div>
 ...
 onChange={(e) => onChange({ props: { ...node.props,  badgeText: e.target.value } })} 
 />
