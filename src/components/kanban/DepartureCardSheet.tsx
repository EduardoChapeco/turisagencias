...
function CheckinCartoesSection({ card }: { card: DepartureCardData }) {
 ...
 toast.success('Cartão de embarque anexado com sucesso!');
 fetchPasses();
 } catch (err: any) {
 ...
 toast.error("Erro ao buscar link: " + err.message);
 } finally {
 setLoading(false);
 }
 };
 ...
 toast.error("Erro ao gerar link do portal: " + err.message);
 } finally {
 ...
 <Button onClick={() => { navigator.clipboard.writeText(portalUrl); toast.success('Link copiado!'); }} size="sm" variant="outline" className="border-indigo-300">
 Copiar
 </Button>
 </div>
 </div>
