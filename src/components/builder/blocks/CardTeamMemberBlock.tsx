...
export const CardTeamMemberBlock = {
  ...
  renderComponent: ({ data }: any) => (
    <div className="p-6 border rounded-xl flex flex-col items-center text-center shadow-sm">
      <div className="w-24 h-24 bg-gray-200 rounded-full mb-4 flex items-center justify-center text-gray-400">
        <User size={32} />
      </div>
      <h3 className="font-semibold text-lg">{data?.name || 'Jane Doe'}</h3>
      <p className="text-blue-600 text-sm mb-4">{data?.role || 'Travel Expert'}</p>
