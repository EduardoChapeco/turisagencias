 ...
 *,
 ...
 <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Nenhuma entrada processada neste período.</TableCell></TableRow>
 ) : (
 entries?.map((entry: any) => (
 <TableRow key={entry.id}>
 <TableCell className="font-medium">{entry.profiles?.full_name || 'Agente'}</TableCell>
 <TableCell>{'Operadora'} <br/><span className="text-xs text-muted-foreground">{entry.sale_reference || 'N/A'}</span></TableCell>
 <TableCell className="text-right">{formatCurrency(entry.sale_gross_amount)}</TableCell>
 <TableCell className="text-right">{formatCurrency(entry.over_net)}</TableCell>
 <TableCell className="text-right text-muted-foreground">{formatCurrency(entry.commission_base)}</TableCell>
 <TableCell className="text-right text-muted-foreground">{formatCurrency(entry.commission_over)}</TableCell>
 <TableCell className="text-right font-bold text-vj-green">{formatCurrency(entry.commission_total)}</TableCell>
