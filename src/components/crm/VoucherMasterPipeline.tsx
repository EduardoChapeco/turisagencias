...
export default function VoucherMasterPipeline({ onClose, initialData }: { onClose?: () => void, initialData?: any }) {
 ...
 link.href = "https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400&family=DM+Sans:wght@400;500;600;700&family=Outfit:wght@400;500;600;700;900&display=swap";
 link.rel = "stylesheet";
 ...
 if (!newWindow) {
 toast.error("Por favor, permita pop-ups no seu navegador para abrir em uma nova aba.");
 return;
 }
 ...
 ${scriptStart}
 <link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400&family=DM+Sans:wght@400;500;600;700&family=Outfit:wght@400;500;600;700;900&display=swap" rel="stylesheet">
 <style>
 body { 
 background: #FDFDFD; 
 ...
 toast.error("Ocorreu um erro ao exportar as imagens.");
 } finally {
 setLoading(false);
 }
 };
 ...
 toast.error("As bibliotecas de exportação ainda estão carregando. Tente novamente em 2 segundos.");
 return;
 }
...
className = {`px-4 py-1.5 rounded-md transition-all flex items-center gap-2 font-semibold text-xs ${activeTab === 'voucher' ? 'bg-white text-slate-800 border border-slate-200' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}
title = "Ver Voucher"
 >
...
className = {`px-4 py-1.5 rounded-md transition-all flex items-center gap-2 font-semibold text-xs ${activeTab === 'embarque' ? 'bg-white text-slate-800 border border-slate-200' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}
...
className = {`px-4 py-1.5 rounded-md transition-all flex items-center gap-2 font-semibold text-xs ${activeTab === 'emergencia' ? 'bg-white text-slate-800 border border-slate-200' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}
...
className = "p-2 rounded-md bg-[#1A2B47] text-white hover:bg-[#2C4A6E] transition flex items-center justify-center "
...

{
 ...
 <button onClick={ handleSave } className = "w-full py-3 bg-vj-green text-white rounded-lg text-sm font-bold hover:bg-vj-green/90 transition " >
...
<div style={ { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 1, background: 'linear-gradient(to bottom, rgba(26,43,71,0.2) 0%, rgba(26,43,71,0.95) 100%)' } }> </div>
 ...
 < div className = "p-6 rounded-3xl border border-white/10 " style = {{ backgroundColor: 'rgba(15, 28, 46, 0.6)' }}>
...
{/* PÁGINA 4: TRANSPORTE */ }
{
 ...
 < div className = "space-y-3" >
 <div className="bg-white p-4 rounded-xl flex gap-3 items-start border-l-[3px] border-[#1A2B47] " >
 ...
 < div className = "bg-white p-4 rounded-xl flex gap-3 items-start border-l-[3px] border-[#6EC3EC] " >
...
{/* PÁGINA 5: PASSEIOS */ }
{
 ...
 {
 p.telefone && (
 <div className="bg-white p-4 rounded-xl flex gap-3 items-center border-l-[3px] border-[#2C4A6E] ">
 <Phone size={ 16} className = "text-[#2C4A6E] shrink-0" />
 <div>
 ...
 =========================================== */}
{
 ...
 < div className = "bg-white p-4 rounded-xl flex gap-3 items-center border-l-4 border-red-500 " >
...
{
 activeTab === 'whatsapp' && (
 <div className="w-full max-w-2xl bg-white rounded-2xl p-6 md:p-8 border border-slate-200 self-start " >
