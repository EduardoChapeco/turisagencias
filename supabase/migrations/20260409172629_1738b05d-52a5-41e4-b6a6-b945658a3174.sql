...
CREATE INDEX idx_quotations_org ON public.quotations(org_id);
CREATE INDEX idx_quotations_client ON public.quotations(client_id);
CREATE INDEX idx_quotations_status ON public.quotations(status);
CREATE INDEX idx_quotations_share_token ON public.quotations(share_token);

CREATE TRIGGER update_quotations_updated_at
  BEFORE UPDATE ON public.quotations
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE POLICY "Users can view quotations in own org"
  ON public.quotations FOR SELECT USING (org_id = public.get_my_org_id());
CREATE POLICY "Users can create quotations in own org"
  ON public.quotations FOR INSERT WITH CHECK (org_id = public.get_my_org_id());
CREATE POLICY "Users can update quotations in own org"
  ON public.quotations FOR UPDATE USING (org_id = public.get_my_org_id());
CREATE POLICY "Users can delete quotations in own org"
  ON public.quotations FOR DELETE USING (org_id = public.get_my_org_id());
...
CREATE POLICY "Users can view quotation sources"
  ON storage.objects FOR SELECT
  USING (bucket_id = 'quotation-sources' AND auth.role() = 'authenticated');
CREATE POLICY "Users can upload quotation sources"
  ON storage.objects FOR INSERT
  WITH CHECK (bucket_id = 'quotation-sources' AND auth.role() = 'authenticated');
CREATE POLICY "Users can delete quotation sources"
